        ...
        <div
            ...
            @if (isset($trips) && $trips->count())
                <section aria-labelledby="trips-heading">
                    ...
                    <h2 id="trips-heading"
                        class="text-lg sm:text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">
                        Your Trips
                    </h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-fr">
                        @foreach ($trips as $trip)
                            <div class="aspect-video  bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex flex-col relative hover:shadow-xl"
                                ...
                                <!-- Overlay -->
                                <div
                                    class="absolute inset-0 bg-gray-700 opacity-30 rounded-lg z-0 transition-opacity duration-300 hover:opacity-30">
                                </div>
                                <div class="relative z-10 flex flex-col h-full">
                                    <h3 id="trip-{{ $trip->id }}-heading"
                                        class="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">
                                        {{ $trip->title }}
                                    </h3>
                                    <ul
                                        class="list-disc list-inside text-gray-700 dark:text-gray-300 space-y-1 text-sm sm:text-base mt-2 flex-grow">
            ...
            @else
                <section aria-labelledby="no-trips-heading">
                    <h2 id="no-trips-heading"
                        class="text-lg sm:text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">
                        No Trips Yet
                    </h2>
                    <p class="text-base sm:text-lg text-gray-700 dark:text-gray-300">Get started by creating your first
                        trip!</p>
                </section>
            @endif
        </div>
